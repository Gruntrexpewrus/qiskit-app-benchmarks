...
class PegasosQsvcBaseClassifierBenchmark(BaseClassifierBenchmark, ABC):
    """Base class for PegasosQSVC benchmark"""
    ...
    def _construct_PegasosQsvc(
        ...
        if method == "quantumclassical":
            ...
            #quantum kernel, not parametrized
            qkernel = QuantumKernel(feature_map=feature_map, quantum_instance=self.backends[quantum_instance_name])
            model = PegasosQSVC(quantum_kernel=qkernel, C=1.0, num_steps=1000, precomputed=False, seed=42)
            return model
        elif method == "quantum":
            #super dumb parametrized start
            #<<<<<<<<<<<<<<<<< any number of qubits
